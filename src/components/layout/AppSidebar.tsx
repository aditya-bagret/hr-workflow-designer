...
const sections: Array<{ title?: string; items: NavItem[] }> = [
  {
    ...
    items: [
      ...
      { icon: <CalendarClock size={14} />, label: 'Scheduler' },
  ...
  },
  {
    title: 'Automation',
    items: [
      { icon: <Plug size={14} />, label: 'Integrations', badge: 3 },
      { icon: <BookOpen size={14} />, label: 'Repository' },
      { icon: <GitMerge size={14} />, label: 'Workflows', active: true },
    ],
  },
  {
    title: 'Resources',
    items: [
      ...
      { icon: <Inbox size={14} />, label: 'Inbox', badge: 12 },
      { icon: <MessageSquare size={14} />, label: 'Messages' },
    ],
  },
];
          ...
          <div key={section.title}>
            ...
            <div className="space-y-0.5">
              {section.items.map(item => (
                <button
                  key={item.label}
                  className="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg transition-all group text-left"
                  style={{
                    background: item.active ? 'rgba(249,115,22,0.1)' : 'transparent',
                    color: item.active ? '#f97316' : '#9ca3af',
                  }}
                  onMouseEnter={e => {
                    if (!item.active) (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.04)';
                    if (!item.active) (e.currentTarget as HTMLButtonElement).style.color = '#e5e7eb';
                  }}
                  onMouseLeave={e => {
                    if (!item.active) (e.currentTarget as HTMLButtonElement).style.background = 'transparent';
                    if (!item.active) (e.currentTarget as HTMLButtonElement).style.color = '#9ca3af';
                  }}
